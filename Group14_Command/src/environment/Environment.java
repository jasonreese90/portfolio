...
public class Environment
{
    ...
    */
     ...
     */
    public static Environment createEnvironment(int width, int height)
    {
    ...
    /**
     * 
     * @param lifeForm the life form to move.
     * @param speed the speed to move at.
     * @return returns true if the life form successfully moved, otherwise returns false.
     */
    public boolean moveLifeForm(LifeForm lifeForm, int speed)
    {
    	speed = Math.min(lifeForm.getMaxSpeed(), Math.abs(speed)-lifeForm.getCurrentSpeed());
    	
    	if(speed > 0)
    	{
    		OrderedPair op = getLifeFormPosition(lifeForm);
    		OrderedPair originalPosition = new OrderedPair(op.x,op.y);
    	
    		if(lifeForm.getCurrentDirection() == LifeForm.DIRECTION_NORTH)
    		{
    			op.y = Math.max(op.y-speed, 0);
    		}
    		else if(lifeForm.getCurrentDirection() == LifeForm.DIRECTION_SOUTH)
    		{
    			op.y = Math.min(op.y+speed, height-1);
    		}
	    	else if(lifeForm.getCurrentDirection() == LifeForm.DIRECTION_EAST)
		    {
	    		op.x = Math.min(op.x+speed, width-1);
	    	}
	    	else if(lifeForm.getCurrentDirection() == LifeForm.DIRECTION_WEST)
	    	{
	    		op.x = Math.max(op.x-speed, 0);
	    	}
    		
    		int offset = Math.abs((op.x-originalPosition.x)+(op.y-originalPosition.y));

    		if(offset > 0)
    		{
    			LifeForm lf = getLifeForm(op.x,op.y);
    			
    			if(lf == null)
    			{
    				removeLifeForm(originalPosition.x,originalPosition.y);
    				addLifeForm(op.x,op.y,lifeForm);
    				lifeForm.setCurrentSpeed(lifeForm.getCurrentSpeed()+offset);
    				return true;
    			}
    		}
    	}
