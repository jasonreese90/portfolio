// -----------------------------------------------------------------------
// <copyright file="SignatureScan.cs" author="Jason Reese" copyright="2010">
// Process signature scan class. Based on boyer-moore string search algorithm.
// </copyright>
// -----------------------------------------------------------------------
...
namespace Kunihiro.Memory.SigScan
{
    ...
    public class SignatureScan
    {
        ...
        public IntPtr FindSignature(string pattern, int offset = 0, int postOffset = 0, bool dereference = true, bool addSigLen = true)
        {
            if (pattern.Length % 2 != 0 || pattern.Length < 2)
                return IntPtr.Zero;
            ...
            if (index != -1)
            {
                ...
                IntPtr addr = (IntPtr)(long)targetModule.BaseAddress + index + len;
                ...
                if (dereference)
                {
                    ...
                    return (IntPtr)(x + postOffset);
                }

                return (IntPtr)(addr + postOffset);
            }

            return IntPtr.Zero;
        }

        public IntPtr FindRelativeSignature(string pattern)
        {
            IntPtr addr = this.FindSignature(pattern, 0, 0,false, true);
            int offset = 0;
            byte[] buffer = new byte[4];

            ReadProcessMemory(process.Handle, addr, buffer, 4, 0);
            offset = BitConverter.ToInt32(buffer, 0);
            return (IntPtr)((long)addr + offset + 4);
        }
